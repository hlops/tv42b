...
class EXTINFItem {
  ...
  constructor(private _name: string, private _url: string, groupOrName: Group|string, shift: number, private _options: Dictionary<string>) {
  ...
  toString(lastGroup) {
    ...
    arr.push.apply(arr, _.map(this._options, (value, key) => {
...
function buildChannels(channels: Dictionary<Channel>, channelItems: Dictionary<ChannelItem>, groups: Dictionary<Group>): EXTINFItem[] {
  const result: EXTINFItem[] = _.compact(_.map(channels, function (channel) {
    ...
    if (channelItem && !channelItem.disabled) {
      ...
      return new EXTINFItem(channelItem.m3u.name, channel.url, group || channelItem.m3u.group, channelItem.shift, channel.options);
    }
  }));
